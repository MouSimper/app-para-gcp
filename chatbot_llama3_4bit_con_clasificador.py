...
def _mark_for_yes(v: str) -> str:
    """Retorna 'X' solo si el valor es 'si'."""
    return "X" if _parse_yesno(v) == "si" else ""

def _mark_for_no(v: str) -> str:
    """Retorna 'X' solo si el valor es 'no'."""
...
def _items_check_table(doc, titulo_seccion, items_with_values):
    ...
    # 1. Crear cabecera con 3 columnas
    t_hdr = doc.add_table(rows=1, cols=3)
    ...
    # 2. Crear tabla de contenido con 3 columnas
    t = doc.add_table(rows=len(items_with_values), cols=3)
    t.alignment = WD_TABLE_ALIGNMENT.CENTER
    
    for i, (label, val) in enumerate(items_with_values):
        # Aplicar sombreado a las 3 celdas
        if i % 2 == 0:
            _set_cell_shading(t.cell(i,0), "F4F6FB")
            _set_cell_shading(t.cell(i,1), "F4F6FB")
            _set_cell_shading(t.cell(i,2), "F4F6FB")
        
        # Columna 0: Descripción
        _cell_p(t.cell(i,0), label, size=10)
        
        # Columna 1: SÍ
        pc_si = t.cell(i,1).paragraphs[0]
        pc_si.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run_si = pc_si.add_run(_mark_for_yes(val)) # <-- Usar _mark_for_yes
        run_si.font.size = Pt(12)
        
        # Columna 2: NO
        pc_no = t.cell(i,2).paragraphs[0]
        pc_no.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run_no = pc_no.add_run(_mark_for_no(val)) # <-- Usar _mark_for_no
...
def build_and_launch_gradio():
    ...
    # Escucha en todas las IPs (0.0.0.0) y en el puerto que Cloud Run espera (8080)
